...
app.get("/authorize", async (c) => {
	...
	return renderApprovalDialog(c.req.raw, {
		...
		server: {
			description:
				"セルフホスト Redmine(redmine.dokkiitech.dev)を操作するリモート MCP サーバー。GitHub でログインして認可する。",
			name: "Redmine MCP",
		},
		setCookie,
		state: { oauthReqInfo },
...
app.get("/callback", async (c) => {
	...
	// 個人用サーバーのため、許可した GitHub アカウント以外は認可を発行しない
	if (login !== c.env.ALLOWED_LOGIN) {
		return c.text(`Access denied: GitHub user "${login}" is not allowed`, 403);
	}
